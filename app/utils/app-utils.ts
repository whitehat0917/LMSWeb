import { AccountStatus, UserInfo } from '@lms-api/models/user-info.model';
...
export const getLayoutRoute = (path: string) => {
  if (path.startsWith(GlobalUrls.ADMIN)) {
    return LayoutRoute.ADMIN;
  }
  if (path.startsWith(GlobalUrls.STUDENT)) {
    return LayoutRoute.STUDENT;
  }
  if (path.startsWith(GlobalUrls.LOGOUT)) {
    return LayoutRoute.LOGOUT;
  }
  if (path.startsWith(GlobalUrls.REGISTRATION)) {
    return LayoutRoute.REGISTRATION;
  }
  if (
    path.startsWith(GlobalUrls.SIGNIN) ||
    path.startsWith(GlobalUrls.SIGNUP) ||
    path.startsWith(GlobalUrls.FORGOT) ||
    path.startsWith(GlobalUrls.NEWPASSWORD)
  ) {
    return LayoutRoute.LOGIN;
  }
  return LayoutRoute.INDEX;
};

export const redirectRoute = (userInfo: UserInfo) => {
  if (userInfo) {
    if (
      !userInfo.organizationId ||
      (userInfo.type === AccountStatus.GUEST && userInfo.addresses.length === 0)
    ) {
      return GlobalUrls.REGISTRATION;
    }
    switch (userInfo.type) {
      case AccountStatus.ADMINISTRATOR:
      case AccountStatus.OWNER:
      case AccountStatus.TEACHER:
        return GlobalUrls.ADMIN;
      case AccountStatus.GUEST:
      case AccountStatus.STUDENT:
        return GlobalUrls.STUDENT;
      default:
        return GlobalUrls.INDEX;
    }
  }
  return GlobalUrls.INDEX;
};

export const redirectErrorRoute = (error: string) => {
  if (error === 'NEWUSER') {
    return GlobalUrls.SIGNUP;
  }
