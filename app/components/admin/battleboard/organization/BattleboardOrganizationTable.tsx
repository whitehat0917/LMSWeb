...
const header = ['Subscriber Name', 'Learners', 'Teams', ''];

export default function BattleboardOrganizationTable(props) {
      ...
      .map((org, index) => (
        <TableRow key={`table-row-${index}`} className={classes.tableRow}>
          ...
          <TableCell className={classes.tableData} align="center">
            <Link href={`/admin/battleboard/${org.id}`} passHref>
              <Button className={classes.tableButton}>
                View Learners and Teams
              </Button>
            </Link>
          </TableCell>
        </TableRow>
